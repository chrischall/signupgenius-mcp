...
1. `pr-auto-review.yml` (reusable pipeline in `chrischall/workflows`) runs a Claude review on every PR **except** the release-please release PR (which it deliberately skips). A `pass` **or** `warn` verdict arms the `ready-to-merge` label; only a `fail` verdict blocks the merge.
...
When the verdict is `warn` or `fail`, the pipeline also opens/updates a single `auto-review-followup` issue capturing the findings (see "Auto-review follow-up issues" below) — `warn` carries nits forward into a later PR; `fail` must be addressed on the PR itself before it can merge.
