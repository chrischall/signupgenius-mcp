...
src/
  ...
  tools/
    ...
    slot-write.ts         # registerSlotWriteTools — claim_slot / release_slot (session-only writes,
                          #   both behind the confirmWrite gate: prompt or confirmToken)
...
15 read + 4 write. Pro-only tools (slot reports) call `client.requireKeyMode(…)`, which throws `KeyModeRequiredError` in session/fetchproxy mode. The public-signup tool needs no auth and works even when `resolveAuth()` has deferred a config error.
...
- **Pro-only report tools.** `report_all`/`report_filled`/`report_available` call `client.requireKeyMode(…)`, which checks the mode BEFORE `requireAccount()`. That ordering matters: plain `requireMode` re-raises the deferred `configError` first, and that error tells the user to sign into the browser — advice that can never enable a key-only endpoint. `requireKeyMode` throws `KeyModeRequiredError` unconditionally — it used to fall through to the shared `ModeMismatchError` whenever an account had resolved, which is the common case, and "Switch to key mode" sent users hunting for a setting rather than telling them key mode means a paid API key. It now always names the tool, both modes, and appends the config error when there is one. These reports are also owner-scoped, so a Pro key is not a workaround for someone else's sheet — use `signupgenius_list_slots`. They still *register* in every mode so Claude knows they exist — only the invocation fails. The v3 web API has no report equivalent (none was found during recon).
...
- **`signupgenius_release_slot` resolves the member id from the session and refuses a mismatch.** `signupgenius_list_slots` publishes every participant's `item_member_id` for any public sheet (it strips `member_id` via `toPublicParticipant` since PRIV-1), so a caller that picked the wrong row could otherwise ask us to withdraw a stranger's sign-up — and the server's authorization for `s.DeletePerson` is unverified. Note the session profile reports the id as `id`, not `memberid`.
...
- **The two slot writes are unverified end-to-end, by choice.** Their payloads were read out of the live Angular `objForm` and `d.ProcessSignUp`, but neither has been executed against the server — doing so would claim and withdraw a slot on a real sheet. All three writes (claim, release, RSVP) go through `confirmWrite` in `src/tools/_shared.ts` — mcp-utils `requireConfirmationWithFallback` + `confirmationFromEnv`: an elicitation prompt where the client supports one, otherwise a preview plus a single-use `confirmToken` bound to the tool, the signed-in account, the target and the exact wire payload. Nothing is written on the first call. Treat the claim payload as high-confidence-but-unproven.
...
- Tool return shape: `textContent` (`tools/_shared.ts`) is `minifiedResult` from `@chrischall/mcp-utils` — `{ content: [{ type: 'text', text: JSON.stringify(...) }] }`, one line, no indentation. The pretty-printed two-space form this line used to document is gone: indentation is tokens the caller pays for and never reads. Whitespace *inside* a value is untouched; only the JSON framing is minified.
- **No `view: compact | full` parameter — a decision, not an omission.** The fleet's `view` rung strips image/avatar URLs out of an upstream payload. Most reads here *are* verbatim SignUpGenius payloads, so the premise holds — but `stripMediaUrls` matches nothing SignUpGenius actually emits: its key rule is anchored at both ends (`^<noun>s?(link|uri|url)s?$`), and the two media fields in our captured fixtures are `profilepicdata` (`tests/fixtures/participants.json`) and `signupimage` (`tests/fixtures/signupinfo.json`), neither of which matches; the URL rule needs an image extension ending the path, and `signupimage` is a bare directory. Measured against both fixtures it removes **0 bytes**. Meanwhile it *would* strip `image` — the one banner URL `toSignUpDetails` (`tools/public-signup.ts`) deliberately keeps, guarding against exactly that bare-directory value. So the rung as written is inert where it would apply and harmful where it would bite. A `src/view.ts` was added by a fleet rollout and removed for this reason. To revisit: capture a real `get_profile` / `list_group_members` / `report_all` payload, check whether `profilepicdata` is a URL or a base64 blob (the name suggests the latter, which changes the arithmetic), then wire a `drop: ['profilepicdata', 'signupimage']` / `keep: ['image']` helper to those specific tools — never blanket.
